    ...
    <div className="max-w-[100vw] overflow-x-hidden min-h-screen bg-[#0a0b0d] text-white font-sans pb-44 px-5">
      ...
      {timer > 0 && (
        <div className="fixed top-0 left-0 right-0 bg-purple-600 z-[150] p-4 flex justify-between items-center border-b border-purple-400">
          <span className="font-black italic text-sm tabular-nums tracking-tighter uppercase">Rest: {Math.floor(timer / 60)}:{(timer % 60).toString().padStart(2, '0')}</span>
          <button onClick={() => setTimer(0)} className="bg-white text-black px-4 py-1 rounded-lg text-[10px] font-black uppercase tracking-tighter">Skip</button>
        </div>
      )}

      <header className="pt-12 mb-4 uppercase italic font-black tracking-tighter">
        <div className="flex justify-between items-center mb-4">
          <h1 className="text-3xl">{view === 'weight' ? 'Body' : view}</h1>
          <button onClick={() => setShowOptions(!showOptions)} className="text-zinc-700 text-[10px] not-italic tracking-widest">Options</button>
        </div>
        ...
        <div className="space-y-4">
          <div className="bg-zinc-900/50 rounded-3xl p-4 border border-zinc-800/50">
            <div className="flex justify-between items-center mb-2 px-1">
              <p className="text-[8px] font-black text-zinc-500 uppercase tracking-widest">Body Weight Progress</p>
              <p className={`text-xs font-black italic ${parseFloat(weightStats.change) >= 0 ? 'text-purple-400' : 'text-red-400'}`}>{weightStats.change}kg</p>
            </div>
            <div className="h-[80px] w-full">
              <svg width="100%" height="100%" viewBox={`0 0 ${graphW} ${graphH}`} preserveAspectRatio="none">
                {weightHistory.length > 1 && <path d={linePath} fill="none" stroke="#a855f7" strokeWidth="3" strokeLinecap="round" />}
                {weightHistory.map((d, i) => (<circle key={d.id} cx={getX(i)} cy={getY(d.value)} r="3" className="fill-purple-500 stroke-2 stroke-black" />))}
              </svg>
            </div>
          </div>
          ...
          <div className="bg-zinc-900 rounded-[40px] p-8 border border-zinc-800 relative shadow-2xl">
        ...
        <div className="space-y-6">
          ...
          <div className="bg-zinc-900 rounded-[40px] p-6 border border-zinc-800 min-h-[400px]">
            {currentPhase.days[historyDayIndex].exercises.map((ex, exIdx) => {
              ...
              return (
                <div key={ex.name} onClick={() => { setDayIndex(historyDayIndex); setExerciseIndex(exIdx); setView('lift'); }} className="border-b border-zinc-800 py-5 last:border-0 px-2">
        ...
        <div className="space-y-6">
          <div className="bg-zinc-900 rounded-[40px] p-6 border border-zinc-800">
            <div className="mb-6 px-2">
               <p className="text-[10px] font-black text-zinc-600 uppercase tracking-widest italic">Body Progress</p>
            ...
            <div className="space-y-2 max-h-[250px] overflow-y-auto no-scrollbar mt-6">
